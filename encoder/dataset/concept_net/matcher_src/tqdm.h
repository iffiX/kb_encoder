// Modified from https://github.com/aminnj/cpptqdm
#ifndef TQDM_H
#define TQDM_H
#include <unistd.h>
#include <chrono>
...
public:
    ...
    void progress(int curr, int tot, const std::string &unit = "it") {
        if(is_tty && (curr%period == 0)) {
            ...
            double div = 1.;
            std::string displayUnit(unit);
            if (avgrate > 1e6) {
                displayUnit.insert(0, "M"); div = 1.0e6;
            } else if (avgrate > 1e3) {
                displayUnit.insert(0, "K"); div = 1.0e3;
            }
            printf("[%4d/%4d | %3.1f %s | %.0fs<%.0fs] ", curr,tot,  avgrate/div, displayUnit.c_str(), dt_tot, peta);
