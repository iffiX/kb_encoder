...
class OpenBookQAWithSearchTrainer(pl.LightningModule):
    ...
    def train_dataloader(self):
        qa_loader = DataLoader(
            dataset=self.dataset.train_qa_dataset,
        ...
        return {"qa": qa_loader, "search": search_loader}
    ...
    def training_step(self, batch: Dict[str, BatchEncoding], batch_idx):
        # answer shape [batch_size, sequence_length]
        search_out = self.model(
            input_ids=batch["search"]["sentence"].to(self.real_device or self.device),
            attention_mask=batch["search"]["mask"].to(self.real_device or self.device),
            labels=batch["search"]["answer"].to(self.real_device or self.device),
        )
        self.log("search_loss", search_out.loss, prog_bar=True, on_step=True)
        if self.current_epoch >= self.config.search_warmup_epochs:
            qa_out = self.model(
                input_ids=batch["qa"]["sentence"].to(self.real_device or self.device),
                attention_mask=batch["qa"]["mask"].to(self.real_device or self.device),
                labels=batch["qa"]["answer"].to(self.real_device or self.device),
            )
            self.log("qa_loss", qa_out.loss, prog_bar=True, on_step=True)
            return search_out.loss + qa_out.loss
        else:
            return search_out.loss

    # noinspection PyTypeChecker
    def validation_step(self, batch: BatchEncoding, _batch_idx, dataloader_idx):
        if (
            dataloader_idx == 1
            and self.current_epoch < self.config.search_warmup_epochs
        ):
            return None
    ...
    def validate_on_every_process(self, outputs):
        ...
        if self.current_epoch >= self.config.search_warmup_epochs:
            ...
            # Prevent lr_scheduler from activating
            qa_metrics = {"accuracy": 0.01 * self.current_epoch}
    ...
    def test_on_main_process(self, outputs):
        _, qa_tokens = collate_and_filter_outputs(outputs[1])
